const profileWallet2 = document.getElementById("profileWallet2")
if (profileWallet2 !== null) {
    const profileWallet2Data = [
        {
            ...
            datasets: [
                {
                    ...
                    data: profileWallet2Data[0].first,
                    lineTension: 0,
                    pointRadius: 0,
                    borderWidth: 2,
                },
    ...
    const ctx = document.getElementById("profileWallet2").getContext("2d")
    const myLine = new Chart(ctx, config)

    const items = document.querySelectorAll("#area-chart-action2 span")
    items.forEach(function (item, index) {
        item.addEventListener("click", function () {
            config.data.datasets[0].data = profileWallet2Data[index].first
            config.data.datasets[1].data = profileWallet2Data[index].second
            myLine.update()
        })
    })
}
